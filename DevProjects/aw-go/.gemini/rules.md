...
When working on this codebase, always read these files in order:

1. **`.claude/ARCHITECTURE.md`** — System architecture, request flow, directory structure, database patterns
2. **`.claude/rules.md`** — This file. Coding standards, conventions, evaluation checklist
3. **`.claude/references/`** — Domain-specific reference files (load only what's relevant — see routing table below)
...
## Reference Files — Consult When Relevant

Before starting a task, determine if any of these reference files apply. Read the relevant ones alongside this file.
...
If the task is general PHP/JS development, this file alone is sufficient.
...
- Use `DbClass::Factory()->DbParams()` to create parameter arrays
...
## 11. SQL & Database Rules

1. **SQL must live in model classes** — never in controllers, helpers, or views
2. **Always use `DbClass::Factory()->DbExecute()`** with prepared statements
3. **Parameters via `DbClass::Factory()->DbParams()`** — pass variables by reference
4. **Pagination via `PaginateSet()`** — defined on `OilClass` in `oilylife/models/oil.php`
5. **Return values**: Always return `GetModelSuccess()` / `GetModelError()` from model methods
6. **Single-row queries**: Use `SuccessOnOnlyOneRow($result)` when expecting exactly one row
7. **Multi-database**: Specify `"db" => "analytics"` (or `cron_tracking`, `service_request`, `zoom_license`, `universal_login`, `domain_tracker`) in `DbExecute()` for non-default databases
8. **Never create new database connections** — the singleton handles connection pooling and HA failover

### Example Pattern
```php
// In a model class:
public function CreateWidget($name, $account_id)
{
    $retval = DbClass::Factory()->DbExecute([
        "sql" => "INSERT INTO widgets (name, account_id, created_at) VALUES (?, ?, NOW())",
        "params" => DbClass::Factory()->DbParams($name, $account_id)
    ]);
