...
For query code patterns and examples, see `.claude/rules.md` Section 6 (PHP Code Patterns).
...
### Staging & Sync Workflows

See `.claude/rules.md` for staging server details, upload procedures, and environment sync workflows.
...
See `oilylife/composer.json` for the current dependency list with versions, and `.claude/references/core-libraries.md` for the full inventory of libraries, helpers, and third-party dependencies.
